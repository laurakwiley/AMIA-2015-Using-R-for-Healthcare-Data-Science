...
```{r read_data}
iwpc_data <- read.delim(file = "iwpc_data_7_3_09_revised.txt") %>% tbl_df()
```
...
Using the [main data set](https://www.pharmgkb.org/download.do?objId=553247439&objCls=SubmissionEvent&ext=.xls) we created our own version of a warfarin dosing algorithm.

### Sex Distribution

The first thing we wanted to look at was the sex distribution of the population.
```{r load data, results = 'asis', echo = FALSE}
data <- read.delim(file = "iwpc_data_7_3_09_revised.txt")
data %>%
  tbl_df() %>% 
  ...
  count(gender) %>% 
  kable(format = "html", table.attr = 'class="pure-table pure-table-bordered"', col.names = c("Sex","Count"))
```